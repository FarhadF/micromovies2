...
RUN cd /go/src/github.com/farhadf/micromovies2/vault  && \
CGO_ENABLED=0 GOOS=linux vgo build -a -installsuffix nocgo /go/src/github.com/farhadf/micromovies2/vault/cmd/server.go
FROM scratch
COPY --from=builder ["/go/src/github.com/farhadf/micromovies2/vault/server", "/"]
ENTRYPOINT ["/server"]